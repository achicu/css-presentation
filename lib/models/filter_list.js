...
(function(){
    ...
    $.extend(FilterList.prototype, {
        ...
        addFilter: function(filterConfig, filterName, fromPreset) {
            if (!filterName)
            ...
            this.fire("filterAdded", [filter, fromPreset]);
            return filter;
        },
        ...
        reload: function(filters) {
            ...
            $.each(filters, function(i, filter) {
                var filterConfig = self.filterStore.loadedOrPendingFilterByName(filter.type),
                    filterItem = self.addFilter(filterConfig, filter.name, true);
                if (filter.active)
                    filterItem.setActive(true);
            });
            this.fire("filtersReloaded");
        },
