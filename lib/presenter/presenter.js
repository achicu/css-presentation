...
(function() {
    function Presenter(app) {
        ...
        if (window.localStorage) {
            var storageTimeElapsed = window.localStorage.getItem(Presenter.TimeElapsedStorageKey);
            if (storageTimeElapsed)
                this.timeElapsed = parseInt(storageTimeElapsed, 10);
        }
    }
    ...
    $.extend(Presenter.prototype, {
        ...
        updateTimeElapsed: function() {
            if (this.started) {
                ...
                if (window.localStorage)
                    window.localStorage.setItem(Presenter.TimeElapsedStorageKey, this.timeElapsed);
        ...
        resetTimer: function() {
            ...
            this.updateInfoBarMinute();
            if (window.localStorage)
                window.localStorage.removeItem(Presenter.TimeElapsedStorageKey);
        }
    });
