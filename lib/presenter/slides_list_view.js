...
(function() {
    ...
    $.extend(SlidesListView.prototype, {
        ...
        onScroll: function(delta, useAnimation) {
            var index = this.scrollView.deltaToIndex(delta),
                points = this.scrollView.distanceToIndex(delta, index),
                min = index - 1,
                max = index + 1;
            this.scrollView.contentView.forEachChild(function(childView, slideIndex) {
                ...
                if (visible && points) {
                    if (slideIndex == min)
                        distance = points.before;
                    else if (slideIndex == index)
                        distance = points.current;
                    else if (slideIndex == max)
                        distance = points.after;
                    distance = 1 - distance / points.max;
                }
                childView.toggle(visible, distance, useAnimation);
            });
        }
    });
