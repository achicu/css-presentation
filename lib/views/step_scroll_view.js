...
(function() {
    ...
    $.extend(StepScrollView.prototype, {
        ...
        distanceToIndex: function(delta, index) {
            if (index == -1)
                return null;
            var collectX = this.type != Global.ScrollView.VERTICAL;
            var collectY = this.type != Global.ScrollView.HORIZONTAL;
            var transformDrag = delta.transformDrag,
                middPointX = collectX ? (this.width() / 2 - (transformDrag.x + delta.x)) : 0,
                middPointY = collectY ? (this.height() / 2 - (transformDrag.y + delta.y)) : 0;
            var stepPointBefore = index > 0 ? this.stepPoints[index - 1] : null,
                stepPoint = this.stepPoints[index],
                stepPointAfter = (index < this.stepPoints.length - 2) ? this.stepPoints[index + 1] : null;
            var result = { current: null, before: null, after: null, max: 0, min: 0 };
            result.current = dist(middPointX, middPointY, stepPoint.x, stepPoint.y);
            result.max = result.current;
            if (stepPointBefore) {
                result.before = dist(middPointX, middPointY, stepPointBefore.x, stepPointBefore.y);
                result.max = Math.max(result.max, result.before);
            }
            if (stepPointAfter) {
                result.after = dist(middPointX, middPointY, stepPointAfter.x, stepPointAfter.y);
                result.max = Math.max(result.max, result.after);
            }
            return result;
        },
