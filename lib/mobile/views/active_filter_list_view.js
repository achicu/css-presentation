    ...
    $.extend(ActiveFilterListView.prototype, {
        ...
        updateBottomButtons: function(delta, useAnimation) {
            ...
            if (useAnimation && delta.x) {
                var timeDragView = Math.max(0.0001, this.listView.time * (dragViewX - oldDragViewX) / delta.x);
                var timeAddFilterButtonView = Math.max(0.0001, this.listView.time * (addFilterButtonViewX - oldFilterButtonViewX) / delta.x);
