    ...
    $.extend(ActiveFilterListView.prototype, {
        ...
        updateBottomButtons: function(delta, useAnimation) {
            ...
            if (useAnimation && delta.x) {
                var timeDragView = this.listView.time * (dragViewX - oldDragViewX) / delta.x;
                var timeAddFilterButtonView = this.listView.time * (addFilterButtonViewX - oldFilterButtonViewX) / delta.x;
                var transitionDragView = Global.Utils.prefixOne("transform") + " " + (timeDragView / 1000) + "s linear";
                var transitionAddFilterButtonView = Global.Utils.prefixOne("transform") + " " + (timeAddFilterButtonView / 1000) + "s linear";
                this.dragView.el.css(Global.Utils.prefixOne("transition"), transitionDragView);
                this.addFilterButtonView.el.css(Global.Utils.prefixOne("transition"), transitionAddFilterButtonView);
            } else {
                this.dragView.el.css(Global.Utils.prefixOne("transition"), "none");
                this.addFilterButtonView.el.css(Global.Utils.prefixOne("transition"), "none");
            }
            this.dragView.el.css(Global.Utils.prefixOne("transform"), "translate3d(" + dragViewX + "px, 0px, 0px)");
            this.addFilterButtonView.el.css(Global.Utils.prefixOne("transform"), "translate3d(" + addFilterButtonViewX + "px, 0px, 0px)");
        },
