    ...
    $.extend(ActiveFilterListView.prototype, {
        ...
        updateBottomButtons: function(delta, useAnimation) {
            ...
            if (useAnimation && delta.x) {
                timeDragView = Math.min(this.listView.time, this.listView.time * (dragViewX - oldDragViewX) / delta.x);
                timeAddFilterButtonView = Math.min(this.listView.time, this.listView.time * (addFilterButtonViewX - oldFilterButtonViewX) / delta.x);
            }

            if (timeDragView > 0)
                this.dragView.el.css(transformProperty, Global.Utils.prefixOne("transform") + " " + (timeDragView / 1000) + "s linear");
            else
                this.dragView.el.css(transformProperty, "none");

            if (timeAddFilterButtonView > 0)
                this.addFilterButtonView.el.css(transformProperty, Global.Utils.prefixOne("transform") + " " + (timeAddFilterButtonView / 1000) + "s linear");
            else
                this.addFilterButtonView.el.css(transformProperty, "none");
