...
(function() {
    ...
    function autoLength(length) {
        return (length == "auto") ? 0 : parseFloat(length);
    }
    ...
    $.extend(ImageView.prototype, {
        ...
        onAnimationFrame: function() {
            ...
            if (this.fire("transformchange", [transform]))
                return;
            transform.originX += autoLength(this.el.css("left"));
            transform.originY += autoLength(this.el.css("top"));
        ...
        onTouchDragMove: function(transform, touch) {
            if (this.canUseDrag) {
                ...
                this.el.css(Global.Utils.prefixOne("transform"), "translate(" + transform.dragX + "px, " + transform.dragY + "px) translateZ(0px) " + this.startTransform);
            }
        },
