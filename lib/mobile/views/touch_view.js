...
(function() {
	...
	$.extend(TouchView.prototype, {
		...
		installTouchTrackingEvents: function() {
			...
			this.el.unbind("touchstart", this.touchEvents.onTouchStart);
		},
		...
		removeTouchTrackingEvents: function() {
			...
			this.el.bind("touchstart", this.touchEvents.onTouchStart);
		},
		...
		installMouseTrackingEvents: function() {
			...
			this.el.unbind("mousedown", this.touchEvents.onMouseDown);
		},
		...
		removeMouseTrackingEvents: function() {
			...
			this.el.bind("mousedown", this.touchEvents.onMouseDown);
		},
		...
		onTouchMove: function(event) {
			...
			for (var i = 0; i < touches.length; ++i) {
				...
				if (!internalTouch) {
					console.error("Invalid touch move event identifier", touch.identifier);
					continue;
				}
		...
		onTouchEnd: function(event) {
			...
			for (var i = 0; i < touches.length; ++i) {
				...
				var internalTouch = this.findTouch(touch.identifier);
				if (!internalTouch) {
					console.error("Invalid touch end event identifier", touch.identifier);
					continue;
				}
				internalTouch.currentPosition = Touch.getPosition(touch);
		...
		onTouchCancel: function(event) {
			...
			for (var i = 0; i < touches.length; ++i) {
				...
				var internalTouch = this.findTouch(touch.identifier);
				if (!internalTouch) {
					console.error("Invalid touch cancel event identifier", touch.identifier);
					continue;
				}
				internalTouch.currentPosition = Touch.getPosition(touch);
		...
		onMouseMove: function(event) {
			...
			if (!internalTouch) {
				console.error("Invalid mouse move event");
				return;
			}
		...
		onMouseUp: function(event) {
			...
			var internalTouch = this.findTouch(Touch.MOUSE);
			if (!internalTouch) {
				console.error("Invalid mouse up event");
				return;
			}
			internalTouch.currentPosition = Touch.getPosition(event.originalEvent);
			internalTouch.state = Touch.END;
