...
(function() {
    ...
    $.extend(ScrollView.prototype, {
        ...
        restoreTransform: function(el) {
            ...
            var time = 500, minAcceleration = 1 / 100;
            var dragX = 0, dragY = 0;
            if (Math.abs(this.scrollAcceleration.x) > minAcceleration ||
                Math.abs(this.scrollAcceleration.y) > minAcceleration) {
                dragX = this.scrollVelocity.x * time + this.scrollAcceleration.x * time * time;
                dragY = this.scrollVelocity.y * time + this.scrollAcceleration.y * time * time;
            }
