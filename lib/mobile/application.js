...
(function() {
    function Application() {
        ...
        function printTouch(type) {
            touchLayerTest.on(type, function(touch) {
                if(!touch) {
                    console.log(type);
                    return;
                }
                if (touch.dragStartTime !== undefined)
                    console.log("drag start time", touch.dragStartTime);
                console.log(type, touch, touch.identifier, touch.currentPosition.pageX, touch.currentPosition.pageY, touch.startPosition.pageX, touch.startPosition.pageY);
            });
        }
        // printTouch("touchstart");
        // printTouch("touchmove");
        // printTouch("touchend");
        // printTouch("touchcanceled");

        printTouch("tap");
        printTouch("longtap");
        printTouch("longtaptimer");
        printTouch("touchdragmove");
        printTouch("touchdragend");


        function printTransform(type) {
            touchLayerTest.on(type, function(transform) {
                if(!transform) {
                    console.log(type);
                    return;
                }
                console.log(type, transform.dragX, transform.dragY, transform.scale, transform.rotation);
            });
        }

        printTransform("touchtransform");
        printTouch("touchtransformend");
        ...
        touchLayerTest.on("touchtransformstart", function() {
            startTransform = cloneTransform(elTransform);
        });
