...
(function() {
    function Application() {
        ...
        var parentImageView = new Global.ImageView();
        parentImageView.installResizeEvent();
        parentImageView.el.css({
            "position": "absolute",
            "left": "0px",
            "top": "0px",
            "width": "100%",
        ...
        var touchLayerTest = new Global.StepScrollView(Global.ScrollView.HORIZONTAL);
        touchLayerTest.setTouchParent(parentImageView);
        touchLayerTest.el.css({
            ...
            "width": "100%",
            "height": "100%"
        }).appendTo(parentImageView.el);
        touchLayerTest.contentEl.css({
            "-webkit-writing-mode": "vertical-lr",
            "height": "100%"
        });
        touchLayerTest.needsToFitInViewport = false;
        touchLayerTest.on("viewselected", function(newView, oldView) {
            if (newView)
                newView.el.css("outline", "5px solid blue");
            if (oldView)
                oldView.el.css("outline", "none");
        });
        window.touchLayerTest = touchLayerTest;
        // touchLayerTest.maxScrollCount = 3;
        for (var i = 0; i < 10; ++i) {
            var touchLayer = new Global.ListItemView();
            touchLayer.setTouchParent(touchLayerTest);
            touchLayer.el.css("display", "inline-block");
            touchLayer.contentEl.css("-webkit-writing-mode", "horizontal-tb");
            touchLayer.el.css("height", "100%");
            for (var j = 0; j < 15; j++) {
                var itemWithTouch = new Global.HighlightTouchView();
                itemWithTouch.setTouchParent(touchLayer);
                touchLayer.contentEl.append(itemWithTouch.el);
                itemWithTouch.el.append(
                    $("<div style='width: 250px; height: 80px; border-radius: 5px; text-align: center; padding-top: 20px; background-color: white; margin-bottom: 5px' />").html("<b>" + i + "</b><br />" + j)
                );
            }
            touchLayerTest.contentEl.append(touchLayer.el);

            var itemWithTouch1 = new Global.HighlightTouchView();
            itemWithTouch1.el.css("display", "inline-block");
            itemWithTouch1.el.css("-webkit-writing-mode", "horizontal-tb");
            itemWithTouch1.setTouchParent(touchLayerTest);
            touchLayerTest.contentEl.append(itemWithTouch1.el);
            itemWithTouch1.el.append(
                $("<div style='width: 250px; height: 80px; border-radius: 5px; text-align: center; padding-top: 20px; background-color: white; margin-bottom: 5px' />").html("<b>" + i + "</b>")
            );
        }

        // touchLayerTest.setScrollIndex(0, false);
    }
